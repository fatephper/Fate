...
if (!defined('SMARTY_DIR')) {
    define('SMARTY_DIR', str_replace('\\','/',dirname(__FILE__) . DS));
}
...
class smarty extends Smarty_Internal_TemplateBase {
    ...
    {		
        ...
        $this->setTemplateDir('.' . DS . 'templates' . DS)
            ...
            ->setConfigDir('.' . DS . 'configs' . DS);
				*/
				$this->setTemplateDir(VIEW_PATH)->setCompileDir(COMPILE_PATH)->setPluginsDir(SMARTY_PLUGINS_DIR);
        $this->setCacheDir(CACHE_PATH)->setConfigDir('.' . DS . 'configs' . DS);
        $this->caching = $config['is_chache'];
        $this->cache_lifetime  = $config['life_time'];
				$this->left_delimiter  = $config['left'];
				$this->right_delimiter = $config['right'];
    ...
    public function createTemplate($template, $cache_id = null, $compile_id = null, $parent = null, $do_clone = true)
    {
        ...
        $compile_id = $compile_id === null ? $this->compile_id : $compile_id;
       	//dump($this->compile_id);
        // already in template cache?
        if ($this->allow_ambiguous_resources) {
        		//echo 9;
            $_templateId = Smarty_Resource::getUniqueTemplateName($this, $template) . $cache_id . $compile_id;
        } else {
        		//echo $this->joined_template_dir;
